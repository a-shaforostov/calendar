...
$('#create-event-ok').on('click', function (event) {
	var form = $('#editevent form')[0];
...
$('#week-placeholder').on('click', '.event', function () {
	...
	var event = eventList.getEvent(eventId);
	var form = $('#editevent form')[0];
...
function selectWeek(date) {
	var baseDate = moment(date).startOf('week');
	weekView.setBaseDate(baseDate);
	weekView.renderGrid();
	weekView.renderWeekEvents();
}

$('.left-week-button').on('click', function () {
	weekView.moveBaseDate(-7);
	weekView.renderGrid();
	weekView.renderWeekEvents();
});

$('.right-week-button').on('click', function () {
	weekView.moveBaseDate(7);
	weekView.renderGrid();
	weekView.renderWeekEvents();
});